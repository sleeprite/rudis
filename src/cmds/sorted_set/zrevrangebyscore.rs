...
impl Zrevrangebyscore {
    pub fn parse_from_frame(frame: Frame) -> Result<Self, Error> {
        ...
        if args.len() < 4 {
            return Err(Error::msg("ERR wrong number of arguments for 'zrevrangebyscore' command"));
        }
        ...
        Ok(Zrevrangebyscore { key, max, min, with_scores, limit })
    }
    ...
    pub fn apply(self, db: &mut Db) -> Result<Frame, Error> {
        match db.records.get(&self.key) {
            Some(Structure::SortedSet(set)) => {
                // 按分数降序：先按升序过滤再反转
                let mut filtered: Vec<(String, f64)> = set.iter()
